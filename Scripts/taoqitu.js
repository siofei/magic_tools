...
const cookie = $.getdata('taoqitu_token')
if (!cookie){
	$.msg($.title, '签到出错', '无cookie')
}
let headers = {
...
async function Checkin() {
	...
	$.log("开始签到");
	await $.get(option, function (error, response, data) {
		if (error) {
			$.log("错误原因：" + error);
			...
			if (body["message"].includes("签到成功")) {
				if (notify) {
					$.msg(title, "签到成功", body["message"]);
				}
			} else {
...
/*********************************
 * environment
 * https://github.com/mimokini/script-chavyleung-for-QX-LOON-SURGE/blob/master/Env.js
 * ********************************
 */
